...
define composer::exec (
  ...
  notify { "composer_${cmd}_${title}" : 
        loglevel => 'notice',
      }

  # if $title == "Wordpress-Install"{
    
  #   exec { "wp_core_install_${cmd}_${title}" :
  #     command => "wp core install --url=wp-composer.dev --title=WP_COMPOSER --admin_user=admin --admin_password=admin --admin_email=wp@gmail.com",
  #     cwd => '/var/www/project/web/wp'
  #   }

  #   exec { "activate_all_plugins_${cmd}_${title}" :
  #     command => "wp plugin activate --all",
  #     cwd => '/var/www/project/web/wp',
  #     require  => Exec["wp_core_install_${cmd}_${title}"]
  #   }

  #   notify { "WP installation: Done!" : 
  #       loglevel => 'notice',
  #       require => Exec["activate_all_plugins_${cmd}_${title}"]
  #     }
  # }
