...
class composer(
  ...
  if $projects or $::execs {
    ...
    # exec { "wp_core_install" :
    #   command => "sudo -u vagrant wp core install --url=wp-composer.dev --title=WP_COMPOSER --admin_user=admin --admin_password=admin --admin_email=wp@gmail.com",
    #   cwd => '/var/www/project/web/wp',
    #   require  => Exec["composer_install_Wordpress-Install" ]
    # }

    # exec { "activate_all_plugins" :
    #   command => "sudo -u vagrant wp plugin activate --all",
    #   cwd => '/var/www/project/web/wp',
    #   require  => Exec["wp_core_install" ]
    # }

    # notify { "WP installation: Done!" : 
    #     loglevel => 'notice',
    #     require => Exec["activate_all_plugins"]
    #   }
}
