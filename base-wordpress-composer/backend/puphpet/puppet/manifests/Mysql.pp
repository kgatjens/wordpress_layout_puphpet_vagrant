class puphpet_mysql (
  ...
  each( $databases ) |$key, $database| {
    ...
    notify { "WP databases: ${database['name']}!" : 
        loglevel => 'notice',
      }
    ...
    if $sql != '' {
      ...
      notify { "WP Pass: ${root_password}!" : 
        loglevel => 'notice',
      }

      notify { "DB : mysql -u${user} -p${password} ${name}!" : 
        loglevel => 'notice',
      }

      exec { 'mysql-importdatabase' :
        command => "mysql -uroot -p${root_password} ${name} < \"/vagrant/puphpet/files/backup.sql\"",
        onlyif  => "test -f /vagrant/puphpet/files/backup.sql",
        require     => Mysql_database[$name]
      }
